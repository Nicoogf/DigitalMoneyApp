...
const ConfirmPayPage = () => {
    ...
    const { fetchCards, cardsList, selectedCardId, setSelectedCardId } = useCards();
    const { register, handleSubmit, setValue } = useForm();
    const { selectedService, fetchService, service } = useServices();
    const { payService } = useTransaction();
    const router = useRouter();
    ...
    const onSubmit = (data) => {
        const selectedAmount =
            selectedCardId === 'available_amount'
                ? credentialsUser?.available_amount
                : cardsList.find(card => card.id === selectedCardId)?.amount;

        if (selectedAmount < service?.invoice_value) {
            setErrorMessage("El saldo seleccionado es insuficiente para cubrir el pago.");
            return;
        }
    ...
    return (
        <main className="text-black">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className='w-[90%] mx-auto max-w-[720px] bg-white rounded-xl shadow-md p-8 mt-8'>
                ...
                <button type="submit" className='mt-6 p-3 bg-greenlime text-graydark font-semibold rounded-xl ml-auto block'>
                    Confirmar Selección
                </button>
            </form>
        </main>
